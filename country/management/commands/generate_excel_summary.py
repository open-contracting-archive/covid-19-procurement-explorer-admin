...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        column = 0
        columns = 0
        column_names = ["Country","Total Contracts","Total Amount (USD)","Total Amount (Local currency)","Direct Contracts","Limited Contracts",
        ...
        for country in countries:
            ...
            report = Tender.objects.filter(country__country_code_alpha_2=country.country_code_alpha_2).values('id').aggregate(
                                        ...
                                        not_identified_method_contracts=Count('id',distinct=True,filter=Q(procurement_procedure='not_identified')),
                                        direct_amount=Sum('goods_services__contract_value_usd',distinct=True,filter=Q(procurement_procedure='direct')),
                                        limited_amount=Sum('goods_services__contract_value_usd',distinct=True,filter=Q(procurement_procedure='limited')),
                                        open_amount=Sum('goods_services__contract_value_usd',distinct=True,filter=Q(procurement_procedure='open')),
                                        selective_amount=Sum('goods_services__contract_value_usd',distinct=True,filter=Q(procurement_procedure='selective')),
                                        not_identified_contracts_amount=Sum('goods_services__contract_value_usd',distinct=True,filter=Q(procurement_procedure='not_identified')),
                                        ...
                                        not_identified_contracts_sum=Sum('goods_services__contract_value_usd',distinct=True,filter=Q(status='not_identified')),
                                        active_contracts_sum=Sum('goods_services__contract_value_usd',distinct=True,filter=Q(status='active')),
                                        completed_contracts_sum=Sum('goods_services__contract_value_usd',distinct=True,filter=Q(status='completed')),
                                        cancelled_contracts_sum=Sum('goods_services__contract_value_usd',distinct=True,filter=Q(status='cancelled'))) 
